...
post.get = function () {
    $.ajax({
        method: 'GET',
        url: 'http://127.0.0.1:8000/api/get-posts',
        dataType: 'json',
...
post.getDetail = function (id) {
    $.ajax({
        method: "GET",
        dataType: "json",
        url: 'http://127.0.0.1:8000/api/get-post-detail',
        data: {
            id: id,
        },
        success: function (data) {
            $('#titlePost').val(data.title);
            $('#coverImage').attr('src', `http://127.0.0.1:8000/uploads/images/posts/${data.coverImage}`);
...
post.save = function () {
    ...
    if ($('#formDataPost').valid()) {
        if ($('#postId').val() != 0) {
            $.ajax({
                method: 'POST',
                url: 'http://127.0.0.1:8000/api//post-update',
                dataType: 'json',
                data: fomrPost,
                contentType: false,
                processData: false,
...
post.delete = function (id) {
    bootbox.confirm({
        ...
        callback: function (result) {
            if (result) {
                $.ajax({
                    url: 'http://127.0.0.1:8000/api/post-destroy',
                    method: "DELETE",
                    dataType: "json",
                    data: {
                        id: id,
